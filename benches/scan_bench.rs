...
tmpfs /tmp tmpfs rw,nosuid 0 0";
    ...
    fn mount_lookup_picks_the_longest_matching_prefix() {
        let got = mount_for_path(MOUNTS, Path::new("/mnt/nas/Audiobooks/Author/Book"));
        assert_eq!(
            got,
            Some((
                "cifs".to_string(),
    ...
    fn mount_lookup_falls_back_to_root() {
        let got = mount_for_path(MOUNTS, Path::new("/home/noah/abooks"));
...
//nas/abooks /mnt/nas/Audiobooks cifs vers=3.1.1 0 0";
    ...
    fn mount_lookup_prefers_the_deeper_of_two_nested_mounts() {
        let got = mount_for_path(NESTED_MOUNTS, Path::new("/mnt/nas/Audiobooks/Author/Book"));
        assert_eq!(got, Some(("cifs".to_string(), "vers=3.1.1".to_string())));
    }

    #[test]
    fn mount_lookup_returns_none_without_a_match() {
        assert_eq!(mount_for_path("", Path::new("/mnt/x")), None);
    }
    ...
    #[test]
    fn scan_full_counts_dirs_entries_gaps_and_audio() {
        // Pins the same behavior the old `time_walk_full_counts_dirs_...` test
        // pinned, minus the wall-clock. `scan_warm` with a fresh `DirIndex` is
        // the routine the `scan_full` group times.
        let dir = tempfile::tempdir().unwrap();
        ...
        let (folders, stats) = scanner::scan_warm(dir.path(), &settings, &DirIndex::new());
        assert_eq!(stats.dirs_visited, 3); // root, Gap, Covered
        assert_eq!(stats.entries_seen, 6);
        let gaps = folders
            .iter()
            .filter(|f| f.directly_holds_audio && f.missing_ebook)
            .count();
        assert_eq!(gaps, 1);
        let audio: usize = folders.iter().map(|f| f.audio_files.len()).sum();
        assert_eq!(audio, 3);
    }

    #[test]
    fn scan_gaps_reduces_to_flagged_folders() {
        // Pins the reduce step the `scan_gaps` group times on top of the walk.
        let dir = tempfile::tempdir().unwrap();
        touch(&dir.path().join("Gap/01.mp3"));
        touch(&dir.path().join("Covered/01.mp3"));
        touch(&dir.path().join("Covered/Book.epub"));
        ...
        let audio: usize = flagged.iter().map(|f| f.audio_files.len()).sum();
        assert_eq!(audio, 1);
    }

    #[test]
    fn scan_warm_reuses_an_unchanged_tree() {
        // Pins the reuse invariant the `scan_warm` group depends on: after the
        // priming walk the index serves every directory without a listing.
        let dir = tempfile::tempdir().unwrap();
        touch(&dir.path().join("Gap/01.mp3"));
        touch(&dir.path().join("Covered/01.mp3"));
        touch(&dir.path().join("Covered/Book.epub"));
        let settings = bench_settings();
        let index = DirIndex::new();
        let (_, first) = scanner::scan_warm(dir.path(), &settings, &index);
        assert_eq!(first.dirs_reused, 0);
        let (folders, second) = scanner::scan_warm(dir.path(), &settings, &index);
    ...
    #[test]
    fn concurrency_levels_defaults_when_unset() {
        // SAFETY: single-threaded test binary for env mutation.
        unsafe { std::env::remove_var(ENV_CONCURRENCY) };
        assert_eq!(concurrency_levels(16), vec![16]);
    }

    #[test]
    fn concurrency_levels_parses_a_comma_list() {
        unsafe { std::env::set_var(ENV_CONCURRENCY, "1,4,8,16") };
        assert_eq!(concurrency_levels(16), vec![1, 4, 8, 16]);
        unsafe { std::env::remove_var(ENV_CONCURRENCY) };
    }

    #[test]
    fn companion_serializes_expected_keys() {
        let comp = Companion {
            tool: "scan_bench",
            label: "smb".to_string(),
            host: "kessel".to_string(),
            kernel: "6.8.0".to_string(),
            unix_time: 1783036800,
            build_profile: "release",
            drop_caches: true,
            input_source: InputSource::Root,
            roots: vec![CompanionRoot {
                path: "/mnt/jane-nas/Audiobooks".to_string(),
                fstype: "cifs".to_string(),
                mount_options: "vers=3.1.1".to_string(),
            }],
            criterion_output: "target/criterion/",
        };
        let json = serde_json::to_string(&comp).unwrap();
        assert!(json.contains("\"tool\":\"scan_bench\""));
        assert!(json.contains("\"input_source\":\"root\""));
        assert!(json.contains("\"fstype\":\"cifs\""));
        assert!(json.contains("\"drop_caches\":true"));
    }
}
