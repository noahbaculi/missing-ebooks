...
The `scan_bench` reports in `../benchmarks/` justify the default. Against a local fuse.mergerfs root the full warm walk falls from 119 ms serial to 17 ms at 16 threads, roughly sevenfold; the curve elbows around 8 threads (18 ms) and barely improves out to 32 (16 ms). Source: `../benchmarks/scan-bench-local-jane-core-1781329903.json`. The rewrite costs nothing on the serial path: the parallel code held to one thread matches the pre-rayon serial walk within noise (119 ms either way warm, 340 against 337 ms cold), so the speedup is real concurrency rather than a regressed baseline recovering. 16 threads takes almost all of the local win without oversubscribing a small host.
