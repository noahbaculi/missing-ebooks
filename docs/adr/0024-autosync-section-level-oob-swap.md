...
The byte-equal invariant tested by `src/web/render.rs::tests::section_handle_render_oob_wraps_render_byte_for_byte` extends to cover SSE payloads: the section a tab receives via SSE equals the section it would get from clicking Rescan.
