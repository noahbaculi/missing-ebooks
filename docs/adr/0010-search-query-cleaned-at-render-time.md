...
A search link's `{query}` placeholder is filled with the folder name, cleaned by a pure function (`query::clean_query`) and percent-encoded as the link template is built.

## Decision

The cleaning runs at render time and the cached scan view stores no query string.
