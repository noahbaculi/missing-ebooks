...
Every setting except `library_roots` has a built-in default (the extension lists, the search links, `bind`, `port`, `ttl_seconds`), so the file is optional. The per-deployment knobs are also settable via prefixed env vars (`MISSING_EBOOKS_LIBRARY_ROOTS`, `_BIND`, `_PORT`, `_TTL_SECONDS`); the prefix avoids collisions, notably with the common bare `PORT`. The structured config stays file-only: `search_links` (a list of tables) plus the extension lists, `exclude_globs`, and `excluded_dirs`, because lists and nested tables encode badly in env vars.
