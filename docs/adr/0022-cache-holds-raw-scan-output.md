...
**One store owns substrate plus IO.** `RawViewStore` in `src/state.rs` owns the cache slot, the substrate (`Arc<Config>`, `Arc<ScanSettings>`, per-root `Vec<Arc<DirIndex>>` with the lock inside each `DirIndex`), and the marker file IO. Its async surface is four methods: `current`, `rescan`, `write_mark`, `remove_mark`. `AppState` collapses to `{ config, store }`. `Arc<Config>` is dual-held: the store uses it as scan substrate; `AppState.config` exposes it to handlers that read pure config data (`search_links`, `library_roots`).
...
- ADR-0034 (2026-07-06): "refresh is a client-driven poll". Not folded in, but the reason the Decision block above no longer mentions a `refresh` method on the store, an `autosync` field on `AppState`, or `cookie_name` as a production `Config` handler dependency. The dashboard now polls `GET /refresh`, which reads through `RawViewStore::current`; the SSE loop, its `refresh` store method, the `autosync` field, and the production `cookie_name` were removed. `cookie_name` still exists on `DemoConfig` for the demo session cookie.
