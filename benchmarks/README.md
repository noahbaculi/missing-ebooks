...
`scan_bench` times the real scanner against the configured library roots, cold and warm, and writes a JSON report. The harness is `examples/scan_bench.rs`; the analysis that drives the default `concurrency` value lives in [ADR-0019](../docs/adr/0019-scan-walk-parallel-sized-by-concurrency.md).
...
Replace the path with your own library root:
    ...
    cargo run --release --example scan_bench -- --root /path/to/audiobooks --label local --drop-caches --concurrency 1,4,8,16,32
